...
void fiq_init(void) {
	BCM2835_ST->C1 = BCM2835_ST->CLO + ticks_per_point;
	BCM2835_ST->CS = 2;
	//PUT32(0x2000B210, 0x00000000);
	BCM2835_IRQ->FIQ_CONTROL = 0x80|1;
}
...
int dac_set_rate(int points_per_second) {
	...
	ticks_per_point = 1E6 / points_per_second;
...
void NOINLINE dac_stop(int flags) {
	...
	BCM2835_IRQ->FIQ_CONTROL = 0x00;

	/* Close the shutter. */
	dac_flags &= ~DAC_FLAG_SHUTTER;
	//LPC_GPIO2->FIOCLR = (1 << DAC_SHUTTER_PIN);
	bcm2835_gpio_clr(SHUTTER_GPIO_PIN);
...
static void inline dac_write_point(dac_point_t *p) {
	...
	//LPC_SSP1->DR = (p->u2 >> 4);
	dac_write_all(MASK_XY(x),  MASK_XY(y), p->i >> 4, p->r >> 4, p->g >> 4, p->b >> 4);
}
...
void shutter_set(int state) {
	if (state) {
		...
		if (dac_control.state == DAC_PLAYING) {
			...
			bcm2835_gpio_set(SHUTTER_GPIO_PIN);
			dac_flags |= DAC_FLAG_SHUTTER;
		}
		__enable_fiq();
	} else {
...
void print_dac_cycle_count(void) {
	uint32_t count   = dac_control.count;
	outputf("dac [%.2d][%.4d][%.2d]", dac_cycle_count, count - count_prev, ticks_per_point);
	count_prev = count;
}
#if 1
...
//void c_fiq_handler(void) {
	...
	uint32_t st_stamp = BCM2835_ST->CLO;
#endif
	BCM2835_ST->CS = 2;
	BCM2835_ST->C1 = BCM2835_ST->CLO + ticks_per_point;

	if (dac_control.irq_do != IRQ_DO_BUFFER)
		return;

	uint16_t consume = dac_control.consume;

	if (consume == dac_control.produce) {
		dac_stop_underflow();
		return;
	}
	...
	uint32_t xi = point->x, yi = point->y;

	int32_t x = translate_x(xi, yi);
	int32_t y = translate_y(xi, yi);

	uint8_t red = ((point->irg) >> 16) & 0x00FF;
	uint8_t green = ((point->irg) >> 4) & 0x00FF;
	uint8_t blue = ((point->bf) >> 4) & 0x00FF;
	uint8_t intensity = (UNPACK_I(point)) >> 8;

	dac_write_all(MASK_XY(x), MASK_XY(y), intensity, red, green, blue);

#if DAC_INSTRUMENT_TIME
	dac_cycle_count = BCM2835_ST->CLO - st_stamp;
#endif
}
#endif
INITIALIZER(hardware, dac_init);

void foo(packed_point_t *point) {
	uint16_t intensity = (UNPACK_I(point)) >> 8;
	//uint16_t red = ((point->irg) >> 16) & 0x00FF;

	//dac_control.red_buffer[dac_control.color_control.red_produce] = red;
	//red = dac_control.red_buffer[dac_control.color_control.color_consume];

	bcm2835_spi_write(intensity << 4 | 0x3000 | (0<<15));
	//bcm2835_spi_write((red << 4) | 0x3000 | (1<<15));

	//uint16_t green = ((point->irg) >> 4) & 0x00FF;
	//uint16_t blue = ((point->bf) >> 4) & 0x00FF;

	//dac_control.green_buffer[dac_control.color_control.green_produce] = green;
	//green = dac_control.green_buffer[dac_control.color_control.color_consume];

	//dac_control.blue_buffer[dac_control.color_control.blue_produce] = blue;
	//blue = dac_control.blue_buffer[dac_control.color_control.color_consume];

	//bcm2835_spi_write((green << 4) | 0x3000 | (0<<15));
	//bcm2835_spi_write((blue << 4)  | 0x3000 | (1<<15));

	return;
}

void foo_b(packed_point_t *point) {
	uint16_t blue = ((point->bf) >> 4) & 0x00FF;
	bcm2835_spi_write((blue << 4)  | 0x3000 | (1<<15));
	return ;
}

void foo_r(packed_point_t *point) {
	uint16_t red = ((point->irg) >> 16) & 0x00FF;
	bcm2835_spi_write((red << 4) | 0x3000 | (1<<15));
	return ;
}

void foo_g(packed_point_t *point) {
	uint16_t green = ((point->irg) >> 4) & 0x00FF;
	bcm2835_spi_write((green << 4) | 0x3000 | (0<<15));
	return ;
}
